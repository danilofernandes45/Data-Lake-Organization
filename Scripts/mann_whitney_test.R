for (i in c(100, 300, 500)){
    for (j in 1:10) {
        ...
        data_fastsa = read.csv(paste('../Data/Performance/Socrata/SA/', toString(i), '/perform-', toString(i), '-', toString(j), '-2.csv', sep = ''), header = FALSE)
        # print(wilcox.test(x = data_sa$V2, y = data_ng$V2, paired = FALSE, alternative = 'greater'))
        print(wilcox.test(x = data_fastsa$V2, y = data_sa$V2, paired = FALSE, alternative = 'greater'))
    }
}
