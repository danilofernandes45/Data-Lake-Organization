...
void Organization::update_effectiveness()
{
    ...
    #if DEBUG
        cout << "\n\nEffectiveness: " << this->effectiveness << endl; ;
    #endif
}
...
Organization* Organization::generate_basic_organization(Instance * inst, float gamma)
{
    ...
    org->root->sum_vector = vector<double>(inst->embedding_dim); // VECTOR INITIALIZED WITH ZEROS
    org->root->sample_size = 0;
    org->root->domain = vector<int>(inst->total_num_columns);
    for (int i = 0; i < inst->total_num_columns; i++)
        org->root->domain[i] = 1;
    ...
    for (int i = 0; i < inst->num_tables; i++)
    {
        ...
        for (int j = 0; j < inst->tables[i]->ncols; j++)
        {
            ...
            state->update_id = -1;
            state->domain = vector<int>(inst->total_num_columns);
...
void Organization::build_leaves()
{
    State *state;
    int count = 0;
    int table_id, col_id;
    vector<double> sum_vector_s;

    //Leaf nodes (columns representation) creation
    for (int i = 0; i < this->instance->num_tables; i++)
    {
        for (int j = 0; j < this->instance->tables[i]->ncols; j++)
        {
            state = new State;
            state->update_id = -1;
            state->abs_column_id = count;
            state->reach_probs = new double[ this->instance->total_num_columns ];
            state->similarities = new double[ this->instance->total_num_columns ];
            state->domain = vector<int>(this->instance->total_num_columns);
            state->domain[count] = 1;
            count++;

            state->sum_vector = this->instance->tables[i]->sum_vectors[j];
            state->sample_size = this->instance->tables[i]->nrows;

            for (int s = 0; s < this->instance->total_num_columns; s++)
            {
                table_id = this->instance->map[s][0];
                col_id = this->instance->map[s][1];
                sum_vector_s = this->instance->tables[table_id]->sum_vectors[col_id];
                state->similarities[s] = cossine_similarity(state->sum_vector, sum_vector_s);
            }
...
Organization* Organization::build_organization_BRKGA(Instance * inst, float gamma, const unsigned p, const double pe, const double pm, const double rhoe, const unsigned K, const unsigned MAXT)
{
    Organization *org = new Organization;
    org->instance = inst;
    org->gamma = gamma;
    ...
    vector<State*> states = vector<State*>(org->leaves);
    vector<State*> new_states;
    int state_id = -org->leaves.size();
    
    while( states.size() > 1 )
    {
        const long unsigned rngSeed = 0;	                // seed to the random number generator
		MTRand rng(rngSeed);				                // initialize the random number generator
        Decoder decoder(states, org->leaves, ceil( 0.01*states.size() ));			// initialize the decoder
		
		// initialize the BRKGA-based heuristic
        // const unsigned chromosome_size = states.size()*(states.size() - 1);
        const unsigned chromosome_size = states.size() * ceil( 0.01*states.size() );
		BRKGA< Decoder, MTRand > algorithm(chromosome_size, p, pe, pm, rhoe, decoder, rng, K, MAXT);

		algorithm.evolve(10);	// evolve the population for one generation
		
        // cout << "Best solution found has objective value = "
		// 		<< -algorithm.getBestFitness() << endl;

		vector<double> best_solution = algorithm.getBestChromosome();
        ...
        solution clustering = decoder.decode_solution(best_solution);
        vector< vector<double> > centroids = get<0>(clustering);
		vector< vector<int> > clusters = get<1>(clustering);
        vector< vector<int> > domain = get<2>(clustering);

        for(unsigned i = 0; i < centroids.size(); i++) {
			if(clusters[i].size() > 1) {
                parent_state = new State;
                parent_state->update_id = -1;
                parent_state->abs_column_id = state_id;
                state_id--;
                parent_state->sum_vector = centroids[i];
                parent_state->domain = domain[i];
                parent_state->reach_probs = new double[ org->leaves.size() ];
                parent_state->similarities = new double[ org->leaves.size() ];

                for (int l = 0; l < org->leaves.size(); l++)
                    parent_state->similarities[l] = cossine_similarity(parent_state->sum_vector, org->leaves[l]->sum_vector);


                for(unsigned k = 0; k < clusters[i].size(); k++) {
                    child_state = states[ clusters[i][k] ];
                    parent_state->children.push_back( child_state );
                    child_state->parents.push_back( parent_state );
                }
                
				// for (unsigned r = 0; r < centroids[i].size(); r++) {
				// 	cout << centroids[i][r] << " ";
                // }
				// cout << "\n";
                new_states.push_back(parent_state);	
			}
            else if(clusters[i].size() == 1){
                new_states.push_back( states[ clusters[i][0] ] );
            }
		}
        states = new_states;
        new_states.clear();
